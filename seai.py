...
def prediction(image, model):
    ...
    [predictions] = tf.keras.applications.imagenet_utils.decode_predictions(
        predictions, 2)
    return predictions
...
a = st.sidebar.selectbox(
    'Navigation:', ["Upload de l'image", "Carte", "Statistiques"])
...
if a == "Upload de l'image":
    ...
    uploaded_file = st.file_uploader(
        "Choisissez une image", type=['png', 'jpg'])
...
if a == "Carte":
    ...
    m = folium.Map(location=[
                   46.232192999999995, 2.209666999999996], zoom_start=6, tiles="Stamen Terrain")
...
if a == "Statistiques":
    ...
    n = row.split(',')
    max_dechet = n[-4]
    # Permet de trouver le nombre total de déchets traités
    ...
    fig.suptitle('Nombre total de déchets traités :' +
                 ' ' + max_dechet, fontsize=22)
    st.pyplot(fig)
